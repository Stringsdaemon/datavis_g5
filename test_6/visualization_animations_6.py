...
def animate_chart(top_apps, chart_type="Balkendiagramm", category=None):
    ...
    if start_animation:
        ...
        for i in range(1, 101):  # Mehr Schritte für weichere Animation
            animated_reviews = (pd.to_numeric(top_apps["Reviews"], errors="coerce").fillna(0) * i / 100).astype(int)
            top_apps["Animated_Reviews"] = animated_reviews

            if chart_type == "Balkendiagramm":
                fig = px.bar(
                    top_apps, x="App", y="Animated_Reviews",
                    title=f"📊 Top 10 Apps nach Anzahl der Bewertungen ({category if category else 'Alle Kategorien'})",
                ...
                for j in range(len(top_apps)):
                    ...
                    fig = px.pie(
                        ...
                        title=f"📊 Verteilung der Bewertungen ({category if category else 'Alle Kategorien'})",
                        color_discrete_sequence=px.colors.qualitative.Set2,
                        hole=0.3
                    )
                ...
                fig.update_layout(
                    title=f"📊 Entwicklung der Bewertungen ({category if category else 'Alle Kategorien'})",
                    xaxis_title="App",
